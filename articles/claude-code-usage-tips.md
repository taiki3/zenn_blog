...
- ~~ultrathinkはガンガン使う~~ （現在はultrathinkがデフォルトになりました）
...
~~ultrathinkもガンガン使って大丈夫。明らかに簡単な変更以外は手癖でultrathinkさせよう。~~ （2025年1月以降、ultrathinkがデフォルト設定になったため、特別な入力は不要です）
...
Claude Codeは内部でサブエージェント（Taskツール）を使い、複数の作業を並列で実行できる。これを意識した指示を出すと、処理が大幅に高速化する。

### 並列実行を促す指示

```
以下の3つのファイルをそれぞれ並列で修正して：
- src/api/users.ts
- src/api/orders.ts
- src/api/products.ts
```
...
```
テストとリントを並列で実行して、両方の結果を教えて
```

「並列で」「同時に」といったキーワードを入れると、Claude Codeが複数のサブエージェントを立ち上げて処理してくれる。
...
コードベースの調査は特にサブエージェントと相性が良い。複数の観点から同時に調べさせることで、全体像の把握が速くなる。

### 注意点

- サブエージェントは読み取り専用の作業に向いている
- 同じファイルを複数のサブエージェントが同時に編集するとコンフリクトする
- 依存関係のある作業は順番に実行させる
